			...
			async getVismeDivsForSetup() {
				...
				for (const r of e) {
					const o = r.getAttribute('data-form-id') || '',
						...
						u =
							t.some((c) => c.formId === o) && i === s.constants.mode.FULL_PAGE;
					o &&
						!u &&
						(t.push(l),
						s.forms.push({
			...
			createIframe(
				...
				(a.style.cssText = o),
					...
					a.addEventListener('load', () => {
						const f = e.getAttribute('data-user-custom-data');
						f &&
							a.contentWindow?.postMessage(
								{ type: 'vismeForms:setUserCustomData', id: l, data: f },
								'*'
							);
					}),
					(c.ref = a);
